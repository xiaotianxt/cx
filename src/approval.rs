...
mod tests {
    ...
    fn cancel_worker_removes_pending_and_claimed_approvals() {
        ...
        let pending = broker
            .register(worker.clone(), json!(1), Some(String::from("thread-1")))
            .unwrap();
        let claimed = broker
            .register(worker.clone(), json!(2), Some(String::from("thread-2")))
            .unwrap();
        let other = broker
            .register(other_worker, json!(3), Some(String::from("thread-3")))
            .unwrap();
    ...
    fn cancel_thread_removes_pending_and_claimed_approvals_for_that_thread() {
        ...
        let pending = broker
            ...
            .unwrap();
        let claimed = broker
            .register(worker.clone(), json!(2), Some(String::from("thread-1")))
            .unwrap();
        let other = broker
            .register(worker, json!(3), Some(String::from("thread-2")))
            .unwrap();
